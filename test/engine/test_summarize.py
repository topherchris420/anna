...
def _doc(i, title, abstract, **kw):
    return Document(
        id=f"s:{i}", source="arxiv", kind=DocumentKind.PAPER,
        title=title, abstract=abstract, **kw,
    )
...
class TestExtractiveSummary:
    ...
    def test_to_dict_shape(self):
        docs = [_doc(1, "t", "Some relevant sentence about controllers and stability margins here.")]
...
class TestCompareDocuments:
    def test_comparison_structure(self):
        a = _doc(1, "Kalman filter tutorial", "state estimation with kalman filters",
                 categories=["eess.SY"], version="1")
        b = _doc(2, "Kalman filtering in robotics", "kalman filter for robot localization",
